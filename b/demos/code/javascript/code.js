...
Code.attemptCodeGeneration = function(generator, prettyPrintType) {
  ...
  if (Code.checkAllGeneratorFunctionsDefined(generator)) {
    ...
    content.textContent ="# -*- coding: utf-8 -*-\n"+ code+"\n\n\n";
    if (typeof PR.prettyPrintOne == 'function') {
      code = content.textContent;
      code = PR.prettyPrintOne(code, prettyPrintType);
      //content.innerHTML = code+"<br><br><br>";
    }
  }
};
...
Code.keyInit=function(){
  document.onkeydown=function(event){
    ...
    if(ctrlKey && keyCode == 83) {
      ...
      var name=prompt(MSG['saveinputtitle'],window.sessionStorage.selectname.replace(".t",""));
      if(name==null)return; 
      Code.createAndDownloadFile(text,name,"xml");
    }
   // alert(e.keyCode);
    if(e.keyCode==76)
    {
...
Code.runJS = function() {
  try {
    ...
    if(document.getElementById("runButton").innerHTML.indexOf("play")>0)
    {
     // if(confirm(MSG["reallyrun"]))
      {
      Code.tabClick("debug");
